package com.github.cheukbinli.original.qrcode;
...
public class Qrcode {
    ...
    public boolean[][] calQrcode(byte[] qrcodeData) {
        ...
        /* ---- read version ECC data_template file */
     ...
     *
     * @param content
     * @param imgType  default BufferedImage.TYPE_INT_RGB
     * @return
     * @throws Throwable
     */
    public static byte[] encode(String content, int imgType) throws Throwable {
        if (null == content || content.trim().length() < 1)
            return EMPTY;
        // 获得内容的字节数组，设置编码格式
        byte[] contentBytes = content.getBytes("utf-8");
        if (contentBytes.length > 800) {
            throw new IndexOutOfBoundsException("QRCode content bytes length = " + contentBytes.length + " not in [0, 800].\"");
        }
        ...
        DrawingBoard drawingBoard = createDrawingBoard(imgType, qrcodeHandler.qrcodeVersion, pixoff, codeOut);

        for (int i = 0; i < codeOut.length; i++) {
            for (int j = 0; j < codeOut.length; j++) {
                if (codeOut[j][i]) {
                    drawingBoard.graphics.fillRect(j * 3 + pixoff, i * 3 + pixoff, 3, 3);
                }
            }
        }

        drawingBoard.graphics.dispose();
        drawingBoard.bufferedImage.flush();
        ByteArrayOutputStream output = new ByteArrayOutputStream();
        ImageIO.write(drawingBoard.bufferedImage, "png", output);
//        return Base64.getEncoder().encodeToString(output.toByteArray());
        return output.toByteArray();
    }

    public static byte[] encode(String content) throws Throwable {
        return encode(content, 1);
    }

    public static String getQrcode64(String content) throws Throwable {
        return Base64.getEncoder().encodeToString(encode(content));
    }
    ...
    static class DrawingBoard {
        ...
        public DrawingBoard(BufferedImage bufferedImage, Graphics2D graphics) {
    ...
    public static DrawingBoard createDrawingBoard(int imgType, int size, int pixOff, final boolean[][] codeOut) {
        BufferedImage bufImg = null;
        Graphics2D gs;
        size = size >= 40 ? 40 : size;
        int offset = size < 1 ? 1 : size;
        int imgSize;
        int points = codeOut.length * 3 + pixOff;
//        int points = codeOut.length;
        while ((imgSize = 67 + 12 * (offset - 1)) < points && offset <= 40) {
            offset++;
        }
        bufImg = new BufferedImage(imgSize, imgSize, (imgType < BufferedImage.TYPE_INT_RGB || imgType > BufferedImage.TYPE_BYTE_INDEXED) ? BufferedImage.TYPE_INT_RGB : imgType);
        ...
        return new DrawingBoard(bufImg, gs);
    }


    public static void main(String[] args) throws Throwable {
        System.err.println("data_template:image/png;base64," + getQrcode64("chrome-extension://klbibkeccnjlkjkiokjodocebajanakg/suspended.html#ttl=%E6%9C%89%E9%81%93%E4%BA%91%E7%AC%94%E8%AE%B0&pos=0&uri=https://note.youdao.com/ynoteshare1/index.html?id=dd9ec8a63ab3b97eba58bf7b32e4a8d9&type=notehare1/index.html?id=dd9ec8a63ab3b97eba58bf7b32e4a8d9&type=notehare1/index.html?id=dd9ec8a63ab3b97eba58bf7b32e4a8d9&type=note"));
    }
